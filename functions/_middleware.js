...
export async function onRequest(context) {
    ...
    if (request.method === "POST" && url.pathname === '/webhook') {
        try {
            ...
            // ================================================================
            // 🚨 NEW: Public User Bot Access Control with Control Bot 🚨
            // ================================================================
            if (!botKeyValue) {
...
Bot Key ကို မှန်ကန်စွာ သတ်မှတ်ထားခြင်း မရှိပါ။ ကျေးဇူးပြု၍ Bot Owner ကို ဆက်သွယ်ပါ။
            ...
            // ✅ NEW: Validate BOT_KEY with Control Bot
            try {
...
အသေးစိတ်သိရှိလိုပါက Bot Owner ကို ဆက်သွယ်နိူင်ပါသည်။
                ...
                console.log(`[onRequest] Public User Bot key ${botKeyValue} validated by Control Bot. Response:`, validationResult);
                
            } catch (validationError) {
                console.error("[onRequest] Error validating BOT_KEY with Control Bot:", validationError);
                // If validation fails due to network error, we can either allow or deny.
                // Here we deny access to be safe.
                let chatId = null;
                if (update.message) {
                    chatId = update.message.chat.id;
                } else if (update.callback_query && update.callback_query.message) {
...
ကျေးဇူးပြု၍ နောက်မှ ထပ်မံကြိုးစားပါ သို့မဟုတ် Bot Owner ကို ဆက်သွယ်ပါ။
            ...
            // ================================================================
            // 🚨 END: Public User Bot Access Control with Control Bot 🚨
            // ================================================================
