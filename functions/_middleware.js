...
export async function onRequest(context) {
    ...
    // --- Public User Bot Access Control (Group Bot Style - Full Implementation) ---
    // This block is the exact same logic as the Group Bot's control system.
    // It checks if BOT_DATA is set and validates the key with the Control Bot.
    if (CONTROL_BOT_URL && CONTROL_BOT_URL !== "https://master-control.pages.dev/") {
        // Step 1: Check if BOT_KEY is set
        if (!botKeyValue) {
...
Bot Key ကို မှန်ကန်စွာ သတ်မှတ်ထားခြင်း မရှိပါ။ ကျေးဇူးပြု၍ Bot Owner ကို ဆက်သွယ်ပါ။
        ...
        // Step 2: Validate BOT_KEY with control bot (Exactly like Group Bot)
        try {
            ...
            // Step 3: If validation fails (key invalid or deactivated)
            if (!validationResponse.ok) {
                ...
                if (chatId) {
                    // This is the EXACT SAME message as the Group Bot sends
                    const userFriendlyMessage = `
...
အသေးစိတ်သိရှိလိုပါက Bot Owner ကို ဆက်သွယ်နိူင်ပါသည်။
            ...
            // Step 4: If validation is successful, log it and continue
            ...
            // If control bot is unreachable, you can choose to fail-open or fail-closed.
            // For production, fail-closed is more secure.
